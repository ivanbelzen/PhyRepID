...
output_image = False#Outputs tree as PDF, True for debugging and visualisation, False for production purposes
output_adjusted_repeat_tree = False
...
evo_events_log_file = pre.root+'meme_evo_events_reroot_gt.json'
...
treefix_file_list = glob.glob(pre.denovo_treefix_path + '*' + treefix_ext ) 
...
for treefix_file in treefix_file_list:
	og_id_domain = treefix_file[len(pre.denovo_treefix_path):-len(treefix_ext)]
	...
	#if og_id in evo_events_log['genetree']: continue
...
for treefix_file in treefix_file_list:
	...
	if pre.file_notempty(pre.denovo_treefix_adjusted_images_path+og_id_domain+".pdf"): continue
	...
	if output_image is True:
		...
		img_name = pre.denovo_treefix_adjusted_images_path+og_id_domain+".pdf"
